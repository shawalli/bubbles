...
func (m MonthModel) ViewWeeks() string {
	...
	// Pad end of month
	lastRowDate := time.Date(m.year, m.month, (daysInMonth - len(week)), 0, 0, 0, 0, time.UTC)
	lastRowDate = lastRowDate.AddDate(0, 0, len(week))

	for i := len(week); i < len(m.weekdays); i++ {
		wd := time.Weekday((int(firstVisibleWeekday) + i) % 7)
		day := m.ViewDay(wd, 0, "", true)
		week = append(week, day)
		lastRowDate = lastRowDate.AddDate(0, 0, 1)
	}
	weeks = append(weeks, week)
